...
void NET_DEVICE_Reset(void)
{
	if (GET_STATUS)
	{
		UsartPrintf(USART_DEBUG,"OFF\r\n");
		NET_DEVICE_PWRK_OFF;
		DelayMs(3000);
		while (GET_STATUS)
		{
			UsartPrintf(USART_DEBUG,"ON\r\n");
			NET_DEVICE_PWRK_ON;
			DelayXms(500);
		}
	}
	...
	{
		DelayXms(1000);  //防止是刚关机，先延时后再拉低实现开机
		NET_DEVICE_PWRK_ON;
		while(GET_STATUS)
		{
			;//若没开机，则再次一直循环
...
//	if(!NET_DEVICE_SendCmd("AT+CPOWD=1\r\n", "POWER DOWN", 1))		//关机
//	{
//		
//		NET_DEVICE_PWRK_OFF;
//		DelayMs(1000);
//		NET_DEVICE_PWRK_ON;                   //开机
//		DelayXms(1000);
//		DelayXms(1000);
...
_Bool NET_DEVICE_SendCmd(char *cmd, char *res, _Bool mode)
{
	...
	while(timeOut--)
	{
		if(NET_IO_WaitRecive() == REV_OK)
		{
			...
			{
				UsartPrintf(USART_DEBUG, "netIOInfo.buf:	%s", netIOInfo.buf);
				if(mode)
					NET_IO_ClearRecive();
