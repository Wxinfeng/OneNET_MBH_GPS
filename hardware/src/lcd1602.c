...
//数据、命令控制
#define RS_H	GPIO_SetBits(GPIOB, GPIO_Pin_13)
#define RS_L	GPIO_ResetBits(GPIOB, GPIO_Pin_13)

//读写控制
#define RW_H	GPIO_SetBits(GPIOB, GPIO_Pin_14)
#define RW_L	GPIO_ResetBits(GPIOB, GPIO_Pin_14)

//使能控制
#define EN_H	GPIO_SetBits(GPIOB, GPIO_Pin_15)
#define EN_L	GPIO_ResetBits(GPIOB, GPIO_Pin_15)
...
//	GPIO_Write(GPIOC, value);							//写入GPIOC
	...
	value = GPIO_ReadOutputData(GPIOC);					//读取GPIOC的数据
	value &= ~(0x00FF << 0);							//清除bit0~2
	value |= ((unsigned short)byte & 0x00FF);
	GPIO_Write(GPIOC, value);							//写入GPIOC
...
//    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);				//禁止JTAG功能
	...
	gpioInitStrcut.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3 |GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7;
	gpioInitStrcut.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &gpioInitStrcut);
	
	gpioInitStrcut.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
