...
void Key_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB, ENABLE);	//打开GPIOC、GPIOD的时钟
	
	gpioInitStructure.GPIO_Mode = GPIO_Mode_IPU;									//设置为上拉输入模式
	gpioInitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_12;			//初始化Pin11、12、13
	gpioInitStructure.GPIO_Speed = GPIO_Speed_50MHz;								//承载的最大频率
	GPIO_Init(GPIOB, &gpioInitStructure);											//初始化GPIOC
	
	gpioInitStructure.GPIO_Pin = GPIO_Pin_8;
	GPIO_Init(GPIOA, &gpioInitStructure);											//初始化GPIOD
...
unsigned char Keyboard(void)
{
	...
	if(KeyScan(GPIOB, KEY0) == KEYDOWN)
	{
		...
		while(KeyScan(GPIOB, KEY0) != KEYUP)		//等待释放
			return KEYNONE;
	}
	else
	{
	...
	if(KeyScan(GPIOA, KEY1) == KEYDOWN)
	{
		...
		while(KeyScan(GPIOA, KEY1) != KEYUP)		//等待释放
			return KEYNONE;
	}
	else
	{
	...
	if(KeyScan(GPIOB, KEY2) == KEYDOWN)
	{
		...
		while(KeyScan(GPIOB, KEY2) != KEYUP)		//等待释放
			return KEYNONE;
	}
	else
	{
	...
	if(KeyScan(GPIOB, KEY3) == KEYDOWN)
	{
		...
		while(KeyScan(GPIOB, KEY3) != KEYUP)		//等待释放
			return KEYNONE;
	}
	else
	{
